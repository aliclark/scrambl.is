...
(function () {
	...
	var writeIndex = encryptTo.indexOf('/write/');
	if (writeIndex >= 0) {
	    encryptTo = encryptTo.substring(writeIndex + '/write/'.length);
	    var tweetIndex = encryptTo.indexOf('/tweet/');
	    if (tweetIndex >= 0) {
		document.getElementById('recipient-in').value = decodeURIComponent(encryptTo.substring(tweetIndex + '/tweet/'.length));
		encryptTo = encryptTo.substring(0, tweetIndex);
	    } else {
		var emailIndex = encryptTo.indexOf('/email/');
		if (emailIndex >= 0) {
		    document.getElementById('recipient-in').value = decodeURIComponent(encryptTo.substring(emailIndex + '/email/'.length));
		    encryptTo = encryptTo.substring(0, emailIndex);
		}
	    }
	    if (isTaggedKeyValid(encryptTo)) {
		// because a /write/ URL has only just been pasted in,
